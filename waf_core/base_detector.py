...
Shared plumbing for every detector in waf_core/detectors/. Nothing in
here is attack-specific — that lives in patterns.py and in each
detector's own file. This module owns:

    * pulling raw values out of a WAFRequest (GET, POST, JSON body,
      cookies, headers, file field names) in one consistent shape
...
Identical in behavior to the original waf/detectors/base_detector.py.
The only change from that version is what report() does: the original
called Member 3's log_attack() indirectly through the request cycle
by tagging the result with "_payload" for WAFEngine to pick up later
— that part is unchanged. What's removed is the direct dependency on
Django's request/QueryDict semantics; request.GET/.POST/.COOKIES/etc
are now plain framework-neutral mappings (see request_adapter.py),
but the extraction functions below work identically either way.
"""
...
from waf_core.patterns import REGISTRY, Rule
...
def get_files(request) -> List[str]:
    """Return uploaded file field names only (content inspection is
...
def report(request, result: Dict[str, Any], payload: Any = None) -> None:
    ...
    log line. This does NOT persist anything — persistence (DB rows,
    attacks.log, IP auto-block) is a framework-integration concern
    outside waf_core; see WAFEngine's on_detection hook and
    waf_integration/ for how the Django project wires that back in.
    ...
    engine does, once every detector has run. WAFEngine.inspect()
    collects every detector's result dict (payload included, via the
    "_payload" key stashed here) and invokes the on_detection hook
    itself, once per request, AFTER the decision engine has run — so
    any persistence layer that wires into that hook sees the correct
    final blocked/response_code for every detection in one shot.
